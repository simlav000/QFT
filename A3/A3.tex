...
\end{align}
So the path integral  measure factorizes:
\begin{align}
Z[J]
...
\label{eq:ZJ-GF}
\end{align}
In the second line we completed the square and performed the Gaussian integral
for each mode. The function \(G_F^{(mn)}\) is the \(0{+}1\)D Feynman Green’s
function for frequency \(\omega_{mn}\), defined by
\begin{align}
...
\begin{align}
\langle 0|\phi(t,x,y)\phi(t',x',y')|0\rangle
= \frac{4}{L^2} \sum_{n=1}^\infty\sum_{m=1}^\infty
...
\end{align*}
so that when substituted into \eqref{eq:final-orthonormal}, recalling  that 
$w_{mn}(x, y) = u_m(x) u_n(y)$, we get
\begin{align}
\langle 0|\phi(t,x,y)\phi(t',x',y')|0\rangle
...
\end{align*}
so that when substituted into \eqref{eq:final-orthonormal}, we get
\begin{align}
