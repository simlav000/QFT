...
\begin{align*}
    ...
    &= \prod_{p, a} \sqrt{\frac{-2i\pi V}{\w_a^2 - \w_p^2}}
\end{align*}
...
\end{align*}
As we had seen in class, the only instance in which the integral does not 
vanish, either due to the integrand being odd or due to the terms involving 
Re$(\phi_p^a)^2$ cancelling with the Im$(\phi_p^a)^2$ terms, is when $a = -b$
and $p = q$. Thus we obtain
\begin{align*}
    &= \frac{1}{V^2} \prod_{p, a} \sqrt{\frac{-2i\pi V}{\w_a^2 - \w_p^2}}
    \Bigg(
        \sum_{p, a} \frac{e^{-i\omega_a(t_1 - t_2)} w_p(x_1)w_p(x_2)}{\w_a^2 -
        \w_p^2 + i\epsilon}
    \Bigg)
\end{align*}
Therefore we see the denominator cancels with the product in the numerator, and
we obtain
\begin{align*}
    \frac{\int \mathcal D \phi e^{iS[\phi]}\phi(x_1)\phi(x_2)}{\int \mathcal D
        \phi e^{iS[\phi]}}
        &= \frac{1}{V^2} \sum_{p, a} \frac{e^{-i\omega_a(t_1 - t_2)}
        w_p(x_1)w_p(x_2)}{\w_a^2 - \w_p^2 + i\epsilon}
\end{align*}
